...
Convert this and send it as a mail, add to doc

Using [this great article on Ansible](https://www.openvirtualization.pro/ansible-part-ii-modules-and-ad-hoc-commands/#:~:text=Ansible%20uses%20modules%20to%20perform,by%20any%20shell%20environment%20variables.), I would like to know if I am wrong for thinking that it is wrong to say that there is a "difference" between Ad-hoc and modules, given that I had just been taught in a tutorial that there are 3 ways of pushing in Ansible:
1. Ad Hoc
2. Modules
3. Playbooks

The problem with saying there are "3 ways" is a bit confusing since Ad-Hoc is actually a part of Modules! Basically, Ad Hoc = Using command module. To be more specific, when you run something like `ansible groupName -a "ls"`, what you wrote is being interpreted under the 'command module' secretly:
```
ansible groupName -m command -a "ls" 
```
Both of the above commands would give the exact same output. Therefore, I believe that it would be better to say that. There are 2 ways of pushing in ansible:
1. Modules (Contains Ad Hoc in the form of command module)
2. Playbook (Collection of modules)

Please let me know if I am correct to think this way.
...
- timestamp is the time this data was taken
- type is system is running in vmware, vmname will be hostname

- we can remove some columns
- mac add can be multiple 
- os service can 

- gateway is missing 
- os last patched 
- ntp server is no if it isnt there if yes is there, ntp add should als be there
- mount point should be there in local_disk
- local disk partition is mount point to te folder
- after inventory is done, 
- most info is physical info, then more info needs to be added related to agents
- we have physical and vm serers and k8s clusters too, if 3 info is taken
- anoop sir then gave a demoByYash of how it is to be used
- 2 parts of the prog exists server and client

CREATING THE IMS.CONF COMMANDS

FIRST EXPLANATION GIVEN OF WHERE THE SERVER IS RUNNING and then defined in ims.conf

arun sir:
current excel sheet should have physical details and then config details too - its has 2 step

DEPLOYMENTS
NCLT
HOSTNAME,IP,APP_NAME,OWNER_CONCTACT_DETAIL,APP_DESCRIPTION,DEPLOYMENT_DOCUMENT,DOCUMENT_PATH,DEPLOYMENT_DOCUMENT_TESTED,PUBLIC_URL,BACKUP,BACKUP_URL,LAST_BACKUP_DATE,BACKUP_TESTED,BACKUP_TESTED_DATE
PODS
NCLT
HOSTNAME,IP,APP_NAME,APP_LAUNCH_DATE,LAST_APP_VERSION,GIT_REPO_URL,APP_PLATFORM,PROGRAMMING_LANGUAGE,DATABASE,LOCAL_DB,TIMESTAMP
...
ansible demoByYash -b -m apt -a "name=chessx state=absent" //uninstall app
```
192.168.122.128 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
...
ansible demoByYash -b -m apt -a "pkg=chessx state=present" //installing app
```
192.168.122.128 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
